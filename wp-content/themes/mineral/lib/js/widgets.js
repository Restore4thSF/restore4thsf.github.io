		...
		$.widget("pexeto.pexetoUpload", {
			...
			init: function() {
				...
				this.$fileInput = $('<input />', {
					'type' : 'file',
					'name' : 'pexetofile',
					'class' : 'pexeto-upload-input'})
					.appendTo(this.$form)
					.on('change', this._submitUpload);


				this._positionInputOverBtn(this.$fileInput, this.element);

				this.element.on('click', this._doOnButtonClick);

				this.$form.ajaxForm();

				this.element.on('refresh', function(){
					self._positionInputOverBtn(self.$fileInput, self.element);
				});
			},
			...
			_submitUpload : function(){
				this.element.addClass(this.options.classes.loading);
			...
			_positionInputOverBtn : function($input, $target) {
		        var p, w, h;
			...
			/**
			 * On upload complete callback. If the upload was successfull,
			 * inserts the uploaded image URL in the image URL field. Otherwise,
			 * displays an error.
			 * @param  {string} file     the file name
			 * @param  {object} response the response result
			 */
			_doOnUploadComplete: function(response) {
				var res = $.parseJSON(response),
					errorText = "";
				if(res.success && res.fileurl) {
					//the upload was successful
					fileUrl = res.fileurl;
					this.$inputEl.val(fileUrl);
				} else {
					//the upload was not successful, show an error message
					errorText = res.error || this.options.errorText;
					$("<div />", {
				...
				this.element.removeClass(this.options.classes.loading);
			},
